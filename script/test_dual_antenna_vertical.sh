...
# デュアルアンテナ テスト: 縦向き構成
#   Mainアンテナ: 進行方向に対して前側
#   ロスバッグ: rosbag/rosbag2_2026_05_19-10_50_33_pointcloud
#
...
#
# 実測取付値:
#   Lidar_Y = 0.69m  (base_linkからLiDAR原点のY)
#   IMU_z   = 1.62m  (base_linkからIMUのZ) → LiDAR_Z = 1.62 + 0.3 = 1.92m
#
# センサーキット回転: yaw = +π/2 (90° CCW from above)
#   R(+90°): (xk, yk) → (-yk, xk)
#   main (0, -0.48) → (+0.48,  0) → base_link: ( 0.48, 0.69) ← 前方 ✓
#   sub  (0, +0.48) → (-0.48,  0) → base_link: (-0.48, 0.69) ← 後方 ✓
#   LiDAR yaw: kit_yaw(π/2=1.5708) + internal(1.57096) = 3.14176 ≈ π
...
gnome-terminal -- ros2 run rosbag2_transport player --ros-args \
  -p storage.uri:=rosbag/rosbag2_2026_05_19-10_50_33_pointcloud \
