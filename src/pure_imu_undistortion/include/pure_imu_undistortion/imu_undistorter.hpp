...
private:
  ...
  bool allow_default_speed_fallback_;

  // output/diagnostics
  bool publish_diagnostics_;
  int diag_throttle_ms_;
  int max_pending_clouds_;
  ...
  std::mutex points_callback_mtx_;
  ...
  std::string observed_imu_frame_;
  ...
  struct PointTimeInfo
  {
    ...
    double raw_origin{0.0};      // relative field origin in raw units
    bool interpreted_as_relative{true};
    double t0_sec{0.0};          // scan start
    double t_ref_sec{0.0};       // reference time (start or end)
  };

  enum class CloudProcessResult
  {
    Complete,
    WaitingForFutureSensorData,
  };

  CloudProcessResult processPointCloud(
    const sensor_msgs::msg::PointCloud2::SharedPtr & msg);
  void drainPendingClouds();
  void retryPendingClouds();
  bool shouldWaitForFutureSensorData(
  ...
  bool preparePointTimeInfo(const sensor_msgs::msg::PointCloud2 & msg,
                            PointTimeInfo & out,
  ...
  bool validateCloudLayout(const sensor_msgs::msg::PointCloud2 & msg,
                           std::string & out_reason) const;

  static std::size_t datatypeSize(uint8_t datatype);
