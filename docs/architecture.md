...
- `odom_precision`: isolated local frame used only by the optional precision
  branch.
...
The two nodes must not both publish the same transform. The precision branch
...
In optional precision mode, the odometer additionally publishes exact-key
...
`pure_precision_global_localizer` composes separate precision-local/global
outputs. Neither precision node feeds the production odometer or fusion.
...
## Baseline and isolated precision branch

```text
current scan --> scan-to-scan + fixed-lag SE(2) --> odom -> base_link
                    |
                    `-- accepted-scan snapshot (precision mode only)
                                      |
                                      v
                 ...
                 +--------------------+--------------------+
                 v                                         v
       precision-local output           existing fusion health/anchor
                                                           |
                                                           v
                                             precision-global output
```
...
consumer so it cannot silently jump the precision trajectory.

The precision-global branch accepts an anchor only from a fresh, strictly
...
to the precision target. Repeated external-match rejection rebuilds only the
external target while preserving its last committed correction.

Wheel speed is optional. Hessian-derived directional information, external
precision matching, IMU yaw factors, and GNSS recovery do not depend on a wheel
topic being present. The odometer does not create a binary LiDAR observability
